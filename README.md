...
const me: Blooio.MeRetrieveResponse = await client.me.retrieve();
```
...
const me = await client.me.retrieve().catch(async (err) => {
  if (err instanceof Blooio.APIError) {
...
1. Via the `BLOOIO_LOG` environment variable
...
const client = new Blooio({
  logger: logger.child({ name: 'Blooio' }),
