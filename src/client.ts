...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['BLOOIO_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['BLOOIO_LOG'] or 'warn' if it isn't set.
   */
...
export class Blooio {
   ...
   */
  constructor({
    baseURL = readEnv('BLOOIO_BASE_URL'),
    apiKey = readEnv('BLOOIO_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.BlooioError(
        "The BLOOIO_API_KEY environment variable is missing or empty; either provide it, or instantiate the Blooio client with an apiKey option, like new Blooio({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Blooio.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('BLOOIO_LOG'), "process.env['BLOOIO_LOG']", this) ??
      defaultLogLevel;
  ...
  static BlooioError = Errors.BlooioError;
